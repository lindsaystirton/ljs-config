...
** Turn on inline spell-checking for LaTeX by default
   Requested by Lindsay: flyspell should just be on whenever editing a .tex file,
   rather than something you have to remember to switch on by hand.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (define-advice flyspell-region (:around (orig-fn &rest args) fast-flyspell-region)
    (cl-flet ((sit-for (&rest _) t))
      (apply orig-fn args)))
