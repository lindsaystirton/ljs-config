...
#+begin_src emacs-lisp
  ...
  ;; Requires an external markdown-to-HTML converter on `exec-path'
  ;; (`markdown-command') -- explicitly set to pandoc below, rather
  ;; than left at the default "markdown" tool, since pandoc's own
  ;; Markdown parsing handles tables, footnotes, and citations that
  ;; the plainer default converter doesn't.
  ;;
  ;; The eww preview window opens doubled/split-right, the same way a
  ;; PDF does: `markdown-split-window-direction' defaults to `any',
  ;; which -- because `display-buffer-alist' is non-nil in this config
  ;; -- routes eww's buffer through the shared
  ;; `ljs/display-buffer-doubled-right' (ljs-config-appearance.org)
  ;; rather than a Markdown-specific copy. That entry matches on the
  ;; eww buffer's default name ("*eww*"), so it applies to any eww
  ;; buffer this config opens, not just Markdown previews -- see the
  ;; audit, §25.
  ...
  ;; A Pandoc conversion menu for Markdown and LaTeX buffers, folded
  ;; in from the old, never-loaded ljs46.org (see the audit, §41).
  ;; Also completes a dangling reference already live in
  ;; ljs-config-latex.org's RefTeX setup (:hook (pandoc-mode .
  ;; turn-on-reftex)) -- that hook has had nothing to attach to since
  ;; ljs46.org stopped loading, since pandoc-mode itself was never
  ;; declared anywhere live until now.
