...
#+begin_src emacs-lisp
  (defun ljs-config-compile (&optional arg)
    ...
    (interactive "P")
    ;; `age' is seconds-elapsed-since-modified, so a *larger* number
    ;; means *older* -- `(> (age file) (age el-file))' below reads as
    ;; "the .org source is older than the .el", i.e. the .el is already
    ;; up to date, which is the correct condition to *skip* recompiling.
    ;; (Double-checked after an external review misread this comparison
    ;; as backwards -- it isn't.)
    (cl-flet ((age (file)
                (float-time
...
*** LJS Config Discoverability
which-key and casual -- located in [[file:ljs-config-discoverability.org][ljs-config-discoverability]]
#+name: ljs-config-load-discoverability
#+begin_src emacs-lisp
  (ljs-config-load "./ljs-config/ljs-config-discoverability.org")
#+end_src
...
*** LJS Config Security
GPG-backed file encryption, password-store, and (documented, not
configured) commit signing -- located in [[file:ljs-config-security.org][ljs-config-security]]

#+name: ljs-config-load-security
#+begin_src emacs-lisp
  (ljs-config-load "./ljs-config/ljs-config-security.org")
#+end_src
...
*** LJS Config Email
mu4e/mbsync, currently just the BIEN mailbox pilot -- located in
[[file:ljs-config-email.org][ljs-config-email]]. Safe to load even
before the Terminal-side setup (Homebrew, mbsyncrc, the first sync)
is done -- it quietly no-ops until =mu= is actually installed.

#+name: ljs-config-load-email
#+begin_src emacs-lisp
  (ljs-config-load "./ljs-config/ljs-config-email.org")
#+end_src
...
*** *LJS Config Shell
Located in [[file:ljs-config-shell.org][ljs-config-shell]] -- eshell and vterm.

#+name: ljs-config-load-shell
#+begin_src emacs-lisp
  (ljs-config-load "./ljs-config/ljs-config-shell.org")
#+end_src
...
*** LJS Config Communications
Matrix via ement.el -- located in
[[file:ljs-config-communications.org][ljs-config-communications]]. Safe to load even before your Matrix
password is in =~/.authinfo.gpg= -- it quietly no-ops until
=ljs-matrix-user-id= is set in =ljs-config-private.el=.

#+name: ljs-config-load-communications
#+begin_src emacs-lisp
  (ljs-config-load "./ljs-config/ljs-config-communications.org")
#+end_src

*** LJS Config Claude Code
Claude Code CLI wrapped via claude-code.el (terminal-based, vterm
backend) -- located in [[file:ljs-config-claude-code.org][ljs-config-claude-code]].

#+name: ljs-config-load-claude-code
#+begin_src emacs-lisp
  (ljs-config-load "./ljs-config/ljs-config-claude-code.org")
#+end_src
