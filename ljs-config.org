...
#+begin_src emacs-lisp 
  ...
  ;; Removed: an old defadvice-based filter on package--add-to-archive-contents,
  ;; using the deprecated defadvice mechanism and an internal package.el helper
  ;; whose behaviour has drifted since this was written circa 2013. It only
  ;; ever excluded one package (eieio, a built-in Emacs library that was never
  ;; going to be fetched from MELPA anyway), and strong circumstantial evidence
  ;; suggests it was the reason `package-refresh-contents` silently stopped
  ;; updating the cached MELPA index -- it hadn't actually written a fresh
  ;; archive-contents file to disk since April 2025.
