...
NOTE: this has been through two attempts already -- gating on
=:after stan-mode= and then =:after company= both still produced
"Symbol's function definition is void: company-stan" on startup.
Rather than keep guessing at exactly which package loads when, this
version ties registration to actually entering a stan-mode buffer
(via stan-mode-hook) instead of splicing company-stan into the global
company-backends list at package-load time -- by the time you're in a
stan-mode buffer, everything this needs has necessarily already loaded.
...
#+begin_src emacs-lisp 
  (use-package company-stan
    ...
    :after stan-mode
    :config
    ;; Register company-stan only when actually inside a stan-mode
    ;; buffer (buffer-local), rather than at package-load time.
    (add-hook 'stan-mode-hook
              (lambda () (add-to-list 'company-backends 'company-stan))))
#+end_src
