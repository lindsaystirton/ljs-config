...
NOTE: this used to be =:after stan-mode=, deferring company-stan's own
load (and the add-to-list below) until stan-mode's feature was
actually provided -- which, since stan-mode is itself lazily
autoloaded via =:mode= above, might never happen in a given session.
That looks like the cause of the "Symbol's function definition is
void: company-stan" error: company-backends ended up naming a function
that was never actually loaded. Depending on company (which loads
early and unconditionally) instead avoids the whole timing question --
company-stan is cheap to load immediately, and it's a harmless no-op
backend outside stan-mode buffers anyway.
