...
NOTE: this has been through a few iterations. First, gating on
=:after stan-mode= and then =:after company= both still produced
"Symbol's function definition is void: company-stan" on startup;
fixed by tying registration to actually entering a stan-mode buffer
(via stan-mode-hook) instead of package-load time. Then Company itself
was retired in favour of Corfu (see ljs-config-completion.org) --
Stan has no LSP server, so company-stan (a company-only backend) is
now bridged into the standard completion-at-point-functions mechanism
via Cape's =cape-company-to-capf=, registered buffer-locally so it only
applies inside stan-mode buffers, not globally.
