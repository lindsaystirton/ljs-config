...
(use-package tex
  ...
  ;; Turn on SyncTeX source correlation. Without this, AUCTeX has no
  ;; correlation data to use for forward/backward search between the
  ;; .tex source and the compiled PDF, even though latexmk is already
  ;; generating the .synctex.gz data below -- this was the main
  ;; reason source/PDF syncing never worked.
  (TeX-source-correlate-mode 1)
  ;; Without this, the already-open PDF buffer keeps showing whatever
  ;; it last rendered until you manually press `g' (revert-buffer) in
  ;; it -- compiling again doesn't touch the visible buffer on its
  ;; own. This reverts it automatically the moment a compile finishes,
  ;; so the PDF you're looking at always matches what was just built.
...
NOTE: the actual splitting-and-placing-on-the-right now happens via
=ljs/display-pdf-to-right= (see ljs-config-org.org), invoked
automatically through =display-buffer-alist= whenever a PDF buffer is
shown -- it used to be restricted to org-mode only, which is why this
never worked from LaTeX. The frame-doubling below still runs
pre-emptively before compiling, which is harmless (capped at monitor
width) but is now somewhat redundant with that function.
