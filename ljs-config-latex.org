...
*** Load ebib
    ...
    No bibliography is preloaded here -- the previous setting hardcoded
    a personal path (=/Users/ljs46/...=) to a file Lindsay isn't
    actually using, and it wouldn't have resolved on any other machine
    (or username) anyway. A real per-user bibliography path belongs in
    the per-user override file once that mechanism is rebuilt (see the
    audit); until then, open a bib file manually with =ebib= or =C-c b=
    once point is on a citation key.
