...
** Use pdf-tools with AUCTeX
   - Add this to enable pdf-tools with AUCTeX.
#+begin_src emacs-lisp
...
This function runs stores the screen width as variable  ljs/original-frame-width and runs ljs/frame-double-width before running TeX-command-run-all (bound to C-c C-a). Forces emacs to open the pdf viewer on the right and doubles the width.  The second function reduces the frame-width on killing the right buffer. 
